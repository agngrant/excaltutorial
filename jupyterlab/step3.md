...
To compile and run a specifc example, in this case the conduction example run the following in the BOUT-dev repo directory:
...
Now you can run this using the mpirun command:

`mpirun -n 2 ./conduction`

If you are running this on a machine of your own setup, you may need to vary the `-n 2` for number of processes.

The conduction example should execute now.

And that is BOUT++ in JupyterLab, you can now move on to looking at installing on another type of system and from there move onto using BOUT++ in your own simulations.
